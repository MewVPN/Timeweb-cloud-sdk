...
class ServersService:
    ...
    def builder(self, name: str, preset_id: int) -> ServerBuilder:
        return ServerBuilder(self, name, preset_id)
    ...
    async def reboot(self, server_id: int):
        await self._http.request(
            "POST",
            f"/servers/{server_id}/reboot",
        )

    async def power_off(self, server_id: int):
        await self._http.request(
            "POST",
            f"/servers/{server_id}/shutdown",
        )

    async def hard_power_off(self, server_id: int):
        await self._http.request(
            "POST",
            f"/servers/{server_id}/hard-shutdown",
        )
